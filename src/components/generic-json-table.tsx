...
interface GenericJsonTableProps {
  ...
  /**
   * Explicit Responsibility schema labels win.
   *
   * When absent, schema-less JSON derives columns from the first row.
   */
  ...
  return (
    <div className="flex flex-col space-y-1.5">
      ...
      {media ? (
        ...
        <div className="flex min-h-9 items-center break-words rounded-md border border-border/50 bg-secondary/20 p-2 text-sm font-medium">
          {displayScalar(
            value,
          )}
        </div>
...
export function GenericJsonTable({
  ...
  const resolvedColumns =
    ...
    >(
      () => {
        const source: GenericJsonColumn[] =
          columns &&
          columns.length
            ? columns
            : data.length
    ...
    <div className="space-y-3">
      ...
      >
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[720px]">
          ...
          <DialogDescription>
            Full detail for this record, including captured media.
          </DialogDescription>
